import { WellnessModal } from "../interfaces/wellnessModal";

export const WELLNESS: Record<string, WellnessModal> = {
  entoto: {
    title: "Wellness at Kuriftu Resort & Spa Entoto",
    subtitle: "Wellness at Kuriftu Entoto Adventure Park",
    description:"Beneath the eucalyptus canopy, stillness lingers. A spa shaped by nature’s rhythm invites breath, presence, and gentle return to earth, self, and silence.",
    detailedDescription: "Nestled in the heart of Entoto’s ancient forest, Kuriftu Entoto Forest Spa offers timeless wellness experiences. Guided by decades of expertise, our holistic treatments honor Ethiopia’s rich heritage while empowering local communities. Each therapeutic journey, from natural remedies to curated massage rituals, is a reflection of deep care and mastery—nurturing both body and spirit amidst the tranquil embrace of nature.",
    carouselImages: [
      "https://kuriftu-public-media.s3.amazonaws.com/Entoto/entoto16.jpg",
      "https://kuriftu-public-media.s3.amazonaws.com/Entoto/entoto19.jpg",
      "https://kuriftu-public-media.s3.amazonaws.com/Entoto/entoto17.jpg",
      "https://kuriftu-public-media.s3.amazonaws.com/Entoto/entoto25.webp",
    ],
    wellness: [
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/Entoto/entoto20.jpg",
        title: "Tranquil Escape",
        description:
          "Immerse in the serenity of our Forest Spa, where the soothing scents of eucalyptus and signature house blends guide you into a state of deep relaxation. Our Hot Oil Full Body Massage begins with a ginger-infused foot soak, releasing tension with each gentle stroke. After a day spent exploring the tranquil forest, surrender to this restorative ritual, leaving the body and spirit renewed, balanced, and at peace.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/Entoto/entoto17.jpg",
        title: "Serenity Unveiled",
        description:
          "A day devoted to serenity invites you to unwind, restoring balance to both body and mind. With holistic treatments that nurture your inner calm, this retreat offers more than relaxation—it fosters a renewed sense of clarity. Here, time slows, allowing you to reconnect with yourself, leaving behind the pressures of the outside world. True wellness begins when we take a moment to pause and replenish.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/Entoto/entoto23.jpg",
        title: "The Essence of Rest",
        description:
          "Rooted in a quiet reverence for well-being, each ritual at the Forest Spa is shaped by thoughtful hands and heartfelt care. Guests are invited to explore our curated menu or connect with an intuitive consultant, who will gently guide them toward treatments attuned to personal rhythms—restoring balance through ancient wisdom, native botanicals, and the timeless stillness of the surrounding forest.",
      },
    ],
    gallery: [
      
      
      
    ],

    isLink: true,
    spaMenuLink: "https://kuriftu-public-media.s3.amazonaws.com/files/entotoSpa.pdf",
    items: [
      
    ],

    facilities: [
      "Steam room, sauna, and jacuzzi with refreshing hot and cold drinks",
      "Yoga and meditation place",
    ],
  },
  bishoftu: {
    subtitle: "Relax, Rebalance & Restore",
    title: "Wellness at Kuriftu Resort & Spa Bishoftu",
    description:"Set by Bishoftu’s tranquil lakes, Kuriftu offers a serene escape where nature’s peaceful energy renews the spirit. Whether for family, business, or romance, each stay is unforgettable.",
    detailedDescription: "At Kuriftu, wellness is the essence of our being. From spaces of quiet reflection to invigorating movement and nourishment drawn from nature’s simplicity, every moment is designed to restore balance and inspire vitality. Here, the harmony of body and spirit invites you to embark on a journey toward wholeness, where relaxation and renewal unfold in every detail, empowering you to reach your fullest potential.",
    carouselImages: [
      "https://kuriftu-public-media.s3.amazonaws.com/bishoftu/14.JPG",
      "https://kuriftu-public-media.s3.amazonaws.com/bishoftu/5.jpg",
      "https://kuriftu-public-media.s3.amazonaws.com/bishoftu/15.JPG",
    ],
    wellness: [
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/bishoftu/bishoftu24.webp",
        title: "Rejuvenate, Revive, & Reconnect",
        description: "Journey through the serene landscapes of Bishoftu, where nature’s embrace calms the spirit. In this tranquil haven, nourish both body and soul through mindful experiences, awakening a deeper connection with self and surroundings. Here, every moment becomes an invitation to restore balance, finding joy in simplicity and clarity in stillness, creating a path to wellness in harmony with the land’s timeless beauty.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/bishoftu/bishoftu3.jpg",
        title: "Day of Serenity",
        description: "Escape the demands of daily life, where a day devoted to wellness brings peace to the mind and clarity to your purpose. Here, time pauses, inviting reflection and renewal. In this sanctuary, the art of self-care becomes a personal journey an invitation to disconnect from life’s pressures, restore energy, and return to the rhythm of existence with a renewed sense of balance and purpose.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/bishoftu/14.JPG",
        title: "Blissful Rest Retreat",
        description: "At the heart of our spa is a commitment to your well-being. Here, each treatment is a carefully considered ritual, guided by nature’s wisdom and tailored to your needs. Allow our consultants to offer personalized guidance, or simply immerse yourself in the tranquility of the Forest Spa, where every detail is designed to restore harmony, balance, and a deep sense of renewal.",
      },
    ],
    gallery: [
      "https://kuriftu-public-media.s3.amazonaws.com/bishoftu/14.JPG",
    ],
    isLink: true,
    spaMenuLink: "",
    items: [
      
    ],
    facilities: [
      "Steam room, sauna, and jacuzzi for deep relaxation with refreshing hot and cold drinks",
      "Fully equipped fitness center",
      "Two serene swimming pools designed for both leisure and invigoration",
    ],
  },
  laketana: {
    title: "Wellness at Kuriftu Resort & Spa Lake Tana",
    subtitle: "Wellness at Kuriftu Resort & Spa Lake Tana",
    description:"A serene haven where tranquility and wellness intertwine; Kuriftu Resort & Spa Lake Tana is a place where you are invited to a deeper connection to your self and nature.",
    detailedDescription: "At Kuriftu, wellness is an essential part of the journey. With carefully curated spaces for rest and rejuvenation, opportunities to exert energy, and nourishing, light cuisine, each element of your stay supports a return to balance. The tranquil surroundings and mindful experiences are designed to restore, allowing you to reconnect with yourself and the world, empowering you to accomplish more with clarity and ease.",
    carouselImages: [
      "https://kuriftu-public-media.s3.amazonaws.com/tana/7.webp",
      "https://kuriftu-public-media.s3.amazonaws.com/tana/8.webp",

    ],
    wellness: [
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/tana/7.webp",
        title: "A Journey of Tranquil Restoration",
        description: "Venture into the heart of Lake Tana’s timeless landscapes, where tranquility envelops you. Surrender to the stillness, allowing nature’s embrace to restore and nourish both body and spirit with mindful experiences that ground you in a profound sense of peace.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/tana/7.webp",
        title: "Embrace Stillness, Reclaim Balance",
        description: "Step away from the demands of daily life and immerse yourself in a sanctuary of wellness. Here, each mindful experience restores not only the body but also the spirit, offering a respite that nurtures clarity and focus. In this quiet space, find the grace to reconnect with yourself, cultivating inner peace and balance that extends far beyond your time here.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/tana/8.webp",
        title: "Nourish Your Soul",
        description:"Our spa services are a reflection of our unwavering commitment to your well-being. Each treatment is designed with genuine care, ensuring your health and serenity are prioritized. Explore our Forest Spa menu or reach out to one of our consultants for personalized guidance, helping you choose the perfect experience to nourish your mind, body, and spirit.",
      },
    ],
    gallery: [
      
    ],
    isLink: true,
    spaMenuLink: "https://kurifturesorts.com/lakeTanaSpa.pdf",
    items: [
      
    ],
    facilities: [
      "Steam room, sauna, and jacuzzi",
      "Swimming pool",
    ],
  },
  awash: {
    title: "Wellness at Kuriftu Resort & Spa Awash Falls",
    subtitle: "Reconnect with Nature at Awash Falls",
    description:
      "Our Spa offers unique massage rooms located on the banks of the Awash River, where guests can relax to the sound of the waterfall as their senses are awakened with natural aromas. Our focus is to have guests reconnect with nature at its core with minimal distractions.",
    carouselImages: [
      "https://kuriftu-public-media.s3.amazonaws.com/awash/4.webp",
      "https://kuriftu-public-media.s3.amazonaws.com/awash/5.webp",
      "https://kuriftu-public-media.s3.amazonaws.com/awash/6.webp",
      "https://kuriftu-public-media.s3.amazonaws.com/awash/7.webp",
    ],
    wellness: [
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/awash/5.webp",
        title: "Relax, Rebalance & Restore",
        description: "Our focus is to have guests reconnect with nature at its core with minimal distractions.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/awash/8.webp",
        title: "Full-Day Retreats",
        description: "Take the weekend to soak up the sun, join our sunrise and sunset yoga, lounge in the dipping pool, book an evening massage, and end the day with a beautiful candlelit dinner.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/awash/9.webp",
        title: "Rest Easy Experience",
        description: "Give your body what it ultimately craves - rest.",
      },
    ],
    gallery: [
      
    ],
    isLink: true,
    spaMenuLink: "https://kuriftu-public-media.s3.amazonaws.com/files/AwashSpamenu.pdf",
    items: [
      
    ],
    facilities: [
      "Weekend yoga sessions for relaxation.",
      "Refreshing dipping pool experience.",
      "Luxurious jacuzzi retreat.",
      "Rejuvenating massage therapies.",
      "Exciting dirt biking adventures.",
      "Comfortable outdoor day bed for lounging.",
      "Scenic hiking trails to explore."
    ],
  },
  africanVillage: {
    title: "Wellness at Kuriftu Resort & Spa African Village",
    subtitle: "Wellness at Kuriftu Resort & Spa African Village",
    description:"A sanctuary for stillness and renewal, where gentle treatments, serene spaces, and ancient rhythms restore balance to body, mind, and breath.",
    detailedDescription: "Rooted in Ethiopian tradition and the spirit of African wellness, Kuriftu Resort African Village invites guests to unwind through immersive experiences. Signature massages feature handcrafted oils blended from natural botanicals. The journey continues with Moroccan-inspired rituals, calming steam and sauna sessions, and detailed beauty care—from pedicures and manicures to full salon services. Every treatment is offered in serene, culturally inspired spaces designed to restore balance and inner calm.",
    carouselImages: [
      "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/2.jpg",
      "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/1.jpg",
      "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/5.jpg",
    ],
    wellness: [
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/5.jpg",
        title: "Massage",
        description: "In the quiet embrace of Kuriftu’s sanctuary, hands guided by ancient wisdom trace the contours of tension, dissolving it into the ether. Ethiopian botanicals and rhythmic touch restore equilibrium, as if the highlands themselves whisper renewal. Here, time bends to stillness, and the body remembers its ease.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/7.jpg",
        title: "Steam & Sauna",
        description: "Heat rises like the mist over Lake Tana, purifying skin and spirit. Cedar and eucalyptus weave through the air, a silent hymn to detoxification. In this cloistered warmth, the mind softens, the breath deepens, and the weight of the world slips away—cleansed by steam, reborn in silence.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/3.jpg",
        title: "Gym",
        description: "Strength meets serenity beneath vaulted ceilings, where light spills like honey over polished stone. Ethiopian highland vistas stretch beyond floor-to-ceiling windows, turning movement into meditation. With each rep, the body aligns—honed, not hurried—in a space where discipline and grace converge.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/4.jpg",
        title: "Pedicure",
        description: "Feet, weary from wandering, find solace in ritual. Warm water, fragrant with rosemary and salt, carries away the dust of distant paths. Hands, skilled and unhurried, sculpt nails into moonlit curves, while callouses yield to balms steeped in local honey. A pause, a polish—a return to lightness.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/4.jpg",
        title: "Manicure",
        description: "Fingertips, brushed with gold dust or left unadorned, are tended like sacred art. Cuticles retreat under the press of warmed oil; buffed nails gleam like river stones. The ceremony unfolds in silence, a moment to reclaim grace in the glide of a file, the stroke of a brush.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/6.jpg",
        title: "Swimming Pool",
        description: "Water, cradled by stone and crowned with sky, holds the stillness of a mountain tarn. Sunlight fractures across the surface, painting shifting mosaics on the tiles below. To swim here is to drift between earth and cloud, weightless, as the highland air hums against wet skin.",
      },
      {
        image: "https://kuriftu-public-media.s3.amazonaws.com/africanvillage/wellness/8.jpg",
        title: "Morocco Bath",
        description: "Steeped in North African tradition, the ritual begins in a sanctuary of marble and soft light. Warmth envelops the body as black soap, rich with argan and eucalyptus, lifts away the outer world. A kessa glove glides in rhythmic circles, unveiling skin reborn. Then, rosewater rinses like a desert rain, leaving only silk and sunlight in its wake—a purification both ancient and sublime.",
      },
    ],
    gallery: [
      
      
      
    ],

    isLink: true,
    spaMenuLink: "https://kuriftu-public-media.s3.amazonaws.com/files/entotoSpa.pdf",
    items: [
      
    ],

    facilities: [
      "Massage",
      "Steam andsauna",
      "Gym",
      "Pedicure and manicure",
      "Morocco bath",
      "Swimming pool"
    ],
    
  },
};
