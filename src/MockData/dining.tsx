import { DiningModal } from "../interfaces/diningModal";

export const DINING: Record<string, DiningModal> = {
  entoto: {
    title: "Dining at Kuriftu Entoto Adventure Park",
    subtitle: "kuriftu resort & spa bishoftu dining",
    description:
      "Ethiopia's rich cultural tapestry offers an endless palate of flavors. At Kuriftu Entoto, we blend traditional injera with international dishes, all under one roof. Each restaurant, crafted by our Kuriftu Woodwork team, showcases natural materials like wood, rope, stone, brick, and bamboo, creating a unique dining ambiance.",
    carouselImages: [
      "https://kurifturesorts.com/_nuxt/img/EntotoPeakRestaurant.54d7809.webp",
      "https://kurifturesorts.com/_nuxt/img/s2.147bc76.webp",
      "https://kurifturesorts.com/_nuxt/img/s4.1e7dc28.webp",
    ],
    diningOptions: [
      {
        title: "Entoto Peak Restaurant",
        description:
          "After a day of trekking uphill, unwind at our Mountain Peak Restaurant. This spacious dining hall offers mixed cuisine and various seating areas overlooking the breathtaking landscape.",
        image:
          "https://kurifturesorts.com/_nuxt/img/EntotoPeakRestaurant.54d7809.webp",
        menuLink: "https://kurifturesorts.com/Entoto%20Peak%20Restaurant.pdf",
      },
      {
        title: "The Riders Ranch",
        description:
          "Savor our specialty Chekla Tibs while overlooking the horse stable and track at The Riders Ranch.",
        image:
          "https://kurifturesorts.com/_nuxt/img/TheRidersRanch.2035096.webp",
        menuLink: "https://kurifturesorts.com/RidersRanch2024.pdf",
      },
      {
        title: "Castel Bar & Grill",
        description:
          "Nestled in the forest, our open-air Castel Wine Bar & Grill sits beneath the Rope Course, offering an unparalleled dining experience.",
        image:
          "https://kurifturesorts.com/_nuxt/img/CastelBar&Grill.1f72ad2.webp",
        menuLink: "https://kurifturesorts.com/CastleMenu2024.pdf",
      },
      {
        title: "The Marksman Bar & Grill",
        description:
          "Missed the bullseye? Relax at The Marksman Bar & Grill, serving the best burgers and fries on this side of the mountain.",
        image:
          "https://kurifturesorts.com/_nuxt/img/TheMarksmanBar&Grill.0af1692.webp",
        menuLink:
          "https://kurifturesorts.com/The%20Marksman%20Bar%20&%20Grill.pdf",
      },
      {
        title: "Pit Stop Bar & Grill / Refuel",
        description:
          "After competing with friends, wind down at Pit Stop Bar & Grill. Just remember, the last one in treats the team to a beer!",
        image:
          "https://kurifturesorts.com/_nuxt/img/PitStopBar&Grill.d7a6bea.webp",
        menuLink: "https://kurifturesorts.com/PitStopMenu2024.pdf",
      },
    ],
    diningExperiences: [
      {
        title: "Family Dining",
        description:
          "Enjoy a large family-style dish at any of our restaurant locations—the more, the merrier!",
        image: "https://kurifturesorts.com/_nuxt/img/FamilyDining.7a275ef.webp",
      },
      {
        title: "Archery Challenge",
        description:
          "Think you can hit the target? Bet on it with a friend; the loser buys lunch!",
        image:
          "https://kurifturesorts.com/_nuxt/img/ArcheryCallenge.4956286.webp",
      },
    ],
  },
  bishoftu: {
    title: "Dining at Kuriftu Resort & Spa Bishoftu",
    subtitle: "Kuriftu Resort & Spa Bishoftu Dining",
    description:
      "Ethiopia's diverse culture offers a rich array of flavors. At Kuriftu Resort & Spa Bishoftu, we combine traditional injera with international dishes under one roof. Each restaurant, crafted by our Kuriftu Woodwork team, features natural materials like wood, rope, stone, brick, and bamboo, creating a distinctive dining atmosphere.",
    carouselImages: [
      "https://kurifturesorts.com/_nuxt/img/KuriftuBuffet.82d1f76.webp",
      "https://kurifturesorts.com/_nuxt/img/LakeFront.55a2ad2.webp",
      "https://kurifturesorts.com/_nuxt/img/Lequanda.9dcb7ac.webp",
      "https://kurifturesorts.com/_nuxt/img/WaterparkCabanas.00f0dde.webp",
      "https://kurifturesorts.com/_nuxt/img/WaterparkDiner.e837424.webp",
      "https://kurifturesorts.com/_nuxt/img/Cabbana.911be42.webp",
    ],
    diningOptions: [
      {
        title: "Lakefront Restaurant",
        description:
          "Perfectly Situated over the banks of Lake Kuriftu, our signature Lakefront Restaurant offers a serene dining experience with a stunning lake view.",
        image: "https://kurifturesorts.com/_nuxt/img/LakeFrontF.37dd2a9.webp",
        menuLink: "https://kurifturesorts.com/a%20la%20carte%20menu.pdf",
      },
      {
        title: "The Cabana Restaurant & Bar",
        description:
          "In an ecosystem flourishing with nature, The Cabana Restaurant & Bar includes a special aesthetic of a Japanese inspired Koi Fish Pond. Enjoy a meal amidst this oasis of greenery and relax by the shaded cabanas overlooking the lake",
        image:
          "https://kurifturesorts.com/_nuxt/img/CabbanaF.295b550.webp",
        menuLink: "https://kurifturesorts.com/a%20la%20carte%20menu.pdf",
      },
      {
        title: "Lequanda",
        description:
          "Indulge in our selection of prime meat cuts, including kitfo, tibs, and kurt, at Kuriftu Lequanda, offering a traditional touch.",
        image: "https://kurifturesorts.com/_nuxt/img/LequandaF.73439a0.webp",
        menuLink: "https://kurifturesorts.com/a%20la%20carte%20menu.pdf",
      },
      {
        title: "Waterpark Diner",
        description:
          "Dip and dine at our Waterpark Diner! From standard table structures to outdoor cabanas with a backdrop of the rides, experience a freshing meal with a splash!",
        image:
          "https://kurifturesorts.com/_nuxt/img/WaterparkDinerF.f8a74fa.webp",
        menuLink: "https://kurifturesorts.com/a%20la%20carte%20menu.pdf",
      },
    ],
    diningExperiences: [
      {
        title: "Waterpark Cabanas",
        description:
          "Lay back and relax in our perfectly shaded cabanas while watching your family and friends venture in...",
        image:
          "https://kurifturesorts.com/_nuxt/img/WaterparkCabanasO.a6a031a.webp",
      },

      {
        title: "Kuriftu Buffet",
        description:
          "Serve yourself with our various types of buffets which you can enjoy at our resort...",
        image:
          "https://kurifturesorts.com/_nuxt/img/KuriftuBuffetO.ea5b225.webp",
      },
    ],
  },
  laketana: {
    title: "Dining at Kuriftu Resort & Spa Lake Tana",
    subtitle: "Kuriftu Resort & Spa Lake Tana Dining",
    description:
      "Simple yet elegant, the Tana Lakefront Restaurant caters to the culinary desires of all guests. This naturally structured restaurant appeals to nature enthusiasts seeking an organic aesthetic in every element.",
    carouselImages: [
      "https://kurifturesorts.com/_nuxt/img/3.89ba262.webp",
      "https://kurifturesorts.com/_nuxt/img/2.bf76cc2.webp",
      "https://kurifturesorts.com/_nuxt/img/1.7c07513.webp",
      "https://kurifturesorts.com/_nuxt/img/4.a30d654.webp",
    ],
    diningOptions: [
      {
        title: "Tana Lakefront Restaurant",
        description:
          "This naturally structured restaurant appeals to nature-based travelers who seek a natural aesthetic from every element of the restaurant.",
        image: "https://kurifturesorts.com/_nuxt/img/1.7c07513.webp",
        menuLink: "",
      },
    ],
    diningExperiences: [
      {
        title: "Kuriftu Buffet",
        description:
          "Serve yourself with our various types of buffets, which you can enjoy at our resort—a treat for the eyes and the stomach!",
        image: "https://kurifturesorts.com/_nuxt/img/2.bf76cc2.webp",
      },

      {
        title: "Treat by the porch",
        description: "A treat for the eyes and the stomach!  ...",
        image: "https://kurifturesorts.com/_nuxt/img/3.89ba262.webp",
      },

      {
        title: "Reserve a Table",
        description:
          "Let us know beforehand so that we can save you the best sit in the house....",
        image: "https://kurifturesorts.com/_nuxt/img/4.a30d654.webp",
      },
    ],
  },
  awash: {
    title: "Dining at Kuriftu Resort & Spa Awash Falls",
    subtitle: "KURIFTU RESORT & SPA AWASH FALLS",
    description:
      "We at Kuriftu Awash Falls are delighted to present our latest innovation in the culinary experience",
    carouselImages: [
      "https://kurifturesorts.com/_nuxt/img/TheFallsRestaurant&Bar.69968c3.webp",
      "https://kurifturesorts.com/_nuxt/img/ThePoolBar&Grill.33fc8c3.webp",
      "https://kurifturesorts.com/_nuxt/img/din1.79ddaae.webp",
      "https://kurifturesorts.com/_nuxt/img/din2.2a7cebb.webp",
      "https://kurifturesorts.com/_nuxt/img/din3.10849d7.webp",
    ],
    diningOptions: [
      {
        title: "The Falls Restaurant & Bar",
        description:
          "Designed to highlight our unparalleled view of the park's waterfall, The Falls Restaurant & Bar is decorated with natural light flooding the space, colorful accents, stone structures, and brick elements, creating an organic vibrance for our guests to absorb. From top-brand liquor to signature cocktails and a set menu made up of locally sourced ingredients, our dining experience fully encompasses our guests' needs.",
        image:
          "https://kurifturesorts.com/_nuxt/img/TheFallsRestaurant&Bar.69968c3.webp",
        menuLink: "https://kurifturesorts.com/awashMenu.pdf",
      },
      {
        title: "The Pool Bar & Grill",
        description:
          "We are excited to present our first Dipping Pool experience paired with our bar & grill on an elevated platform overlooking the waterfall. The perfect destination for a romantic getaway with picture-perfect views from sunrise to sunset.",
        image:
          "https://kurifturesorts.com/_nuxt/img/ThePoolBar&Grill.33fc8c3.webp",
        menuLink: "https://kurifturesorts.com/awashMenu.pdf",
      },
    ],
    diningExperiences: [],
  },
  waterpark: {
    title: "Dining at Kuriftu Waterpark",
    subtitle: "KURIFTU WATERPARK",
    description:
      "The dining experience at Kuriftu Resort & Spa restaurant has never been more fulfilling.",
    carouselImages: [
      "https://kurifturesorts.com/_nuxt/img/dinslide.d5ccc8b.webp",
      "https://kurifturesorts.com/_nuxt/img/dinslide2.ca27dc6.webp",
      "https://kurifturesorts.com/_nuxt/img/dinslide3.7e0fcf3.webp",
      "https://kurifturesorts.com/_nuxt/img/dinslide4.8b1e6bb.webp",
      "https://kurifturesorts.com/_nuxt/img/dinslide5.e0dfde1.webp",
      "https://kurifturesorts.com/_nuxt/img/dinslide6.d93b9ac.webp",
    ],
    diningOptions: [
      {
        title: "Plaza",
        description:
          "Dine at our food court beside the cool, fresh water and the fun rides at our water park.",
        image: "https://kurifturesorts.com/_nuxt/img/Plaza.a45b650.webp",
        menuLink: "null",
      },
    ],
    diningExperiences: [
      {
        title: "Kuriftu Buffet",
        description:
          "Serve yourself with our various types of buffets which you can enjoy at our resort.",
        image: "https://kurifturesorts.com/_nuxt/img/KuriftuBuffet.ed0beaf.webp",
      },
      {
        title: "Treat by the Porch",
        description:
          " A treat for the eyes and the stomach!",
        image: "https://kurifturesorts.com/_nuxt/img/TreatByThePorch.df2143d.webp",
      },
      {
        title: "Reserve A table",
        description:
          "Let us know beforehand so that we can save best sit in the house",
        image: "https://kurifturesorts.com/_nuxt/img/ReserveATable.b368639.webp",
      },
    ],
    
  },
};
